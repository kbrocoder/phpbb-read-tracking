...
// KLB need to add the read_history functions
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if ( $error_msg == '' )
			{
				...
				// KLB 10/24/02
				// Need to get the time before posting to make sure the read history
				// time is less than or equal to the actual posting time for
				// comparison
				//
				// It is important to get the time prior to the submit in the
				// case where the submit takes a while and the you get to the
				// next second.  If you don't get the time in the order here
				// the "go to first unread post" (folder and arrow icon) will
				// will not go to the right one but rather miss the one in which
				// the second rolled over.  This is really bad if it was the only
				// unread post by the user.
				//
				$current_time = time();
				...
				// KLB 10/21/02 
				// add the smart read history tracking
				// 
