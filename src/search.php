...
// KLB need to get the read_history functions
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' )  )
		{
			if ( $search_id == 'newposts' )
			{
				...
				{
               // KLB 10/20/02
               // changed the query to include the read history table
					// Note: this query is terrible.  It needs to be re-worked
					// since the read history does all the work it is trying
					// to do now.  Oh well, maybe later.
               //
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			else
			{
				...
				else
				{
					...
					// KLB Interception Point
					// Added for read history tracking
					//
					if ( $userdata['session_logged_in'] )
					{
						// KLB check to see if the topic is new or not
