 ...
 *
 *   $Id: modcp.php,v 1.71.2.5 2002/05/19 15:02:08 psotfx Exp $
 *
 *
 ***************************************************************************/
...
switch( $mode )
{
	case 'delete':
		...
		if ( $confirm )
		{
			...
			// KLB 10/22/02
			// Delete the topic from everyone's read history
			// This should handle the regular topic delete and the case
			// when you delete a topic that has been moved to another forum
			//
		...
		if ( $confirm )
		{
			...
			if ( $new_forum_id != $old_forum_id )
			{
				...
				for($i = 0; $i < count($row); $i++)
				{
					...
					// KLB 10/21/02
					// adjust the forum id in the read history table
					// when the topic is moved to another forum
					// the delete and then submit makes sure that we only add
					// the entry for those that can view the forum it is
					// being moved to (need to re-check authorization)
					//
					$current_time = time();
	...
	case 'split':
		...
		if ( isset($HTTP_POST_VARS['split_type_all']) || isset($HTTP_POST_VARS['split_type_beyond']) )
		{
			...
			// KLB 10/21/02
			// need to mark the new topic as new so people know the topic
			// was split.
			// 
			$current_time = time();
