...
// KLB need the read history functions
...
//
// KLB 10/24/02
// Intercept here to use the post time rather then the user's last login time
// It should give us POST LEVEL TRACKING!!
//
if ( isset($HTTP_GET_VARS['view']) && empty($HTTP_GET_VARS[POST_POST_URL]) )
{
	if ( $HTTP_GET_VARS['view'] == 'newest' )
	{
      ...
      if ( isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_sid']) )
      {
         ...
         {
				// KLB 10/24/02
				// Select all the posts (and take the oldest) from the user's read
	   		...
	   		$sql = "SELECT p.post_id "
				...
				if ( !($result = $db->sql_query($sql)) )
				{
					...
					//message_die(GENERAL_ERROR, 'Could not obtain newer/older topic in formation', '', __LINE__, __FILE__, $sql);
				}

				// KLB 10/24/02
				// There are 2 cases where this will fail
				// 1 - when the topic was moved to another forum (because move 
				//     has to do the auth check again rather than just update the 
				//     forum_id to the new forum_id
				// 2 - when the topic was split.  The post_time is not updated 
				//     in the post table so the submit time in the read history is 
				//     newer than the the actual post time
				//
...
for($i = 0; $i < $total_posts; $i++)
{
   ...
   // Intercept for the icon - KLB
	...
	// KLB 08/28/02
	// changed the if statement to remove all the profile, pm, email
	// crap from the topics
	// line used to be: if ( $poster_id != ANONYMOUS )
	if ( 0 )
	{
...
// KLB mark the topic as read to the user now
// added 11/11/02 - final fix?
//
